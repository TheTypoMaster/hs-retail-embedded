...
public class AddGoods extends Activity {
	...
	private ArrayList allPrice = null;// 一件商品的所有价格，allPrice由singlePrice组成，singlePrice由单位，条形码，进货价，售价组成。
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		allManufacturerSpinner = (Spinner) this
				.findViewById(R.id.allManufacturerSpinner);
		ArrayAdapter<String> allManufacturerAdapter = new ArrayAdapter<String>(
				AddGoods.this, android.R.layout.simple_spinner_item, mCPer
		...
		ArrayAdapter<String> allGoodsKindAdapter = new ArrayAdapter<String>(
				AddGoods.this, android.R.layout.simple_spinner_item,
				goodsKindCPer.getAllGoodsKindName());
		...
		addPriceButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (goodsNameEText.getText().toString().trim().equals("")
						|| goodsNameEText.getText().toString().trim() == null) {
					Toast.makeText(AddGoods.this, "请输入商品的名字",
							Toast.LENGTH_SHORT).show();
				} else {
					Intent intent = new Intent(AddGoods.this,
							AddGoodsPrice.class);// 开始AddGoodsPrice，接收价格信息。
					ArrayList<String> existingUnit = new ArrayList<String>();// 取出allPrice中的所有单位，在AddGoodsPrice中就不显示出那些已经有价格的单位
					ArrayList<String> existingBarcode = new ArrayList<String>();//取出allPrice中的所有条形码，在AddGoodsPrice要进行比较
					for (int i = 0; i < allPrice.size(); i++) {

						SinglePrice temp = (SinglePrice) allPrice.get(i);
						existingUnit.add(temp.get("unitName").toString());
						existingBarcode.add(temp.get("barcode").toString());
					}
					intent.putStringArrayListExtra("existingUnit", existingUnit);
					intent.putStringArrayListExtra("existingBarcode", existingBarcode);

					if(unitCPer.getAllUnitName().size() > existingUnit.size())
						AddGoods.this.startActivityForResult(intent, 0);
					else
						Toast.makeText(AddGoods.this, "已有该商品对于所有单位的价格", Toast.LENGTH_SHORT).show();
				}
		...
		addGoodsOk.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (goodsNameEText.getText().toString().trim().equals("")
					...
					Toast.makeText(AddGoods.this, "请为该商品输入价格",
							Toast.LENGTH_SHORT).show();
				} else {
					Intent intent = new Intent(AddGoods.this,
							PurchaseManagement.class);
					intent.putParcelableArrayListExtra("allPrice", allPrice);// 商品的所有价格
					intent.putExtra("goodsName", goodsNameEText.getText()
							.toString());// 商品名称
					intent.putExtra("mName", allManufacturerSpinner
							.getSelectedItem().toString());// 厂家名称
					intent.putExtra("goodsKind", allKindSpinner
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {// 如果成功接收到数据，数据有单位，条形码，进价，售价
			...
			boolean updateAccordingBarcode = data.getBooleanExtra("updateAccordingBarcode", false);
			if(updateAccordingBarcode){
				
				for(int i=0;i<allPrice.size();i++){
					SinglePrice temp = (SinglePrice)allPrice.get(i);
					if(temp.get("barcode").equals(barcode)){
						SinglePrice replacePrice = new SinglePrice();
						replacePrice.put("unitName", unitName);
						replacePrice.put("barcode", barcode);
						replacePrice.put("inPrice", "" + inPrice);
						replacePrice.put("outPrice", "" + outPrice);
						
						allPrice.remove(i);
						allPrice.add(replacePrice);
					}
				}
			}
			else{
