...
public class SalesBillCPer extends ContentProvider {
	  ...
	  private SQLiteDatabase     sqlDB;
	    ...
	    public List<Integer> getSBillIdByTime(String startTime, String endTime){//根据时间来取得满足的销售单Id
	    	Cursor c = null;
	    	
	    	if(startTime.equals("开始时间") && endTime.equals("结束时间"))//没时间约束
	    		c = this.query(AllTables.SalesBill.CONTENT_URI, null, null, null, null);
	    	else if(!startTime.equals("开始时间") && endTime.equals("结束时间"))//限定开始时间
	    		c = this.query(AllTables.SalesBill.CONTENT_URI, null, " time >= ? ", new String[]{startTime}, null);
	    	else if(startTime.equals("开始时间") && !endTime.equals("结束时间"))//有限定结束时间
	    		c = this.query(AllTables.SalesBill.CONTENT_URI, null, " time <= ? ", new String[]{endTime}, null);
	    	else//有开始时间限制也有结束时间限制
	    		c = this.query(AllTables.SalesBill.CONTENT_URI, null, " time between ? and ? ", new String[]{startTime, endTime}, null);
	    	
	    	
	    	if(c.getCount()>0){//如果根据时间找满足的销售单
	    		List<Integer> salesBillIdList = new ArrayList<Integer>();
	    		c.moveToFirst();
	    		for(int i=0;i<c.getCount();i++){
	    			salesBillIdList.add(c.getInt(0));
	    			c.moveToNext();
	    		}
	    		return salesBillIdList;
	    	}
	    	else
